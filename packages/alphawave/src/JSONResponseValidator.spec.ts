...
describe("JSONResponseValidator", () => {
    ...
    describe("validateResponse", () => {
        ...
        it("should pass a JSON object with schema", async () => {
            const validator = new JSONResponseValidator(schema);
            const response = await validator.validateResponse(memory, functions, tokenizer, { status: 'success', message: { role: 'assistant', content: '{"foo":"bar"}' } }, 3);
            assert.notEqual(response, undefined);
            assert.equal(response.valid, true);
            assert.deepEqual(response.value, { foo: 'bar' });
        });
        ...
        it("should fail a response with no JSON object", async () => {
            const validator = new JSONResponseValidator();
            const response = await validator.validateResponse(memory, functions, tokenizer, { status: 'success', message: { role: 'assistant', content: '' } }, 3);
            assert.notEqual(response, undefined);
        ...
        it("should fail a JSON object that doesn't match schema", async () => {
            const validator = new JSONResponseValidator(schema);
            const response = await validator.validateResponse(memory, functions, tokenizer, { status: 'success', message: { role: 'assistant', content: '{"foo":7}' } }, 3);
            assert.notEqual(response, undefined);
            assert.equal(response.valid, false);
