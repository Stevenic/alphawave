...
describe("PlanValidator", () => {
    ...
    describe("validateResponse", () => {
        it("should pass a response with a valid plan", async () => {
            const validator = new PlanValidator();
            const result = await validator.validateResponse(memory, functions, tokenizer, createResponse(validPlan), 3);
        ...
        it("should pass a response with a valid plan and a valid action", async () => {
            const result = await actionValidator.validateResponse(memory, functions, tokenizer, createResponse(validPlan), 3);
            assert.equal(result.valid, true);
            assert.equal(result.feedback, undefined);
            assert.deepEqual(result.value, validPlan);
        });
        ...
        it("should fail a response with a valid plan but invalid action", async () => {
            const result = await actionValidator.validateResponse(memory, functions, tokenizer, createResponse(invalidPlanAction), 3);
            assert.equal(result.valid, false);
            assert.equal(result.feedback, `The plan JSON is using an Unknown action "lightsOff" for command[0]. Return a JSON object that fixes these errors.`);
        });

        it("should fail a response with a valid plan but missing entity", async () => {
            const result = await actionValidator.validateResponse(memory, functions, tokenizer, createResponse(missingPlanEntity), 3);
            assert.equal(result.valid, false);
        ...
        it("should fail a response with a valid plan but invalid entity", async () => {
            const result = await actionValidator.validateResponse(memory, functions, tokenizer, createResponse(invalidPlanEntity), 3);
            assert.equal(result.valid, false);
            assert.equal(result.feedback, `The plan JSON has invalid entities for action "pause" for command[0]:\n"entities.duration" is not of a type(s) number\n\nReturn a JSON object that fixes these errors.`);
        });

        it("should fail a response with a missing DO action name", async () => {
            const result = await actionValidator.validateResponse(memory, functions, tokenizer, createResponse(missingDOAction), 3);
            assert.equal(result.valid, false);
            assert.equal(result.feedback, `The plan JSON is missing the DO "action" for command[0]. Return a JSON object that fixes these errors.`);
        });

        it("should fail a response with a missing SAY response", async () => {
            const result = await actionValidator.validateResponse(memory, functions, tokenizer, createResponse(missingSAYResponse), 3);
            assert.equal(result.valid, false);
            assert.equal(result.feedback, `The plan JSON is missing the SAY "response" for command[0]. Return a JSON object that fixes these errors.`);
        });
    });
});
